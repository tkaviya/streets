	...
	public void onClick(DialogInterface dialog, int which) {
		switch (which){
			case DialogInterface.BUTTON_POSITIVE:
				...
				if (!askAgain) Startup.getStreetsCommon().setUserPreference("auto_enable_gps", "1"); //auto_enable without asking
				break;

			case DialogInterface.BUTTON_NEGATIVE:
				//No button clicked
                Startup.getStreetsCommon().setUserPreference("suggest_gps", askAgain ? "1" : "0");
				if (!askAgain) Startup.getStreetsCommon().setUserPreference("auto_enable_gps", "0"); //never enable and don't ask
				break;
		}
	}
}
